...
ЁRenan BorgesЁ05/04/16|TUTLR3|Ajuste para gravar corretamente a avalia- Ё
...
Local bRd6GdDelOk		:= { |lDelOk| MyCursorWait() , lDelOk := Rd6GdDelOk( IF( lRd6Status , nOpc , 2 ) ) , MyCursorArrow() , lDelOk }
...
Begin Sequence
	...
	/*/
	здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
	Ё Grava ou Exclui Informacoes do SXM          				   Ё
	юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
	If RDP->RDP_TIPENV != "3" // NЦo envia aviso nem avaliacao
		Rd6PutSxm( ( cOpcao == "DELETE" ) )
	EndIf
...
ЁUso       ЁAPDA270()	                                                Ё
юддддддддддадддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
Static Function Rd6PutSxm( lDelete )
...
Begin Sequence
	...
	bGetNrExclOk	:= { || GetNrExclOk( @cLstCodigo , "SXM" , "XM_CODIGO" , "XM_FILIAL+XM_CODIGO" , { || cCodAux := Soma1( cCodAux ) } , .F. , .F. , cRd6Fil , NIL , .T. ) }
	
	cRd6Fil			:= xFilial( "RD6" )

	cXMAtivo		:= IF( GetMemVar( "RD6_STATUS" ) == "1" , "T" , "F" )

	aRd6SxmInfo		:= Array( 2 , 16 )

	cXmCodigo 		:= "APDS" + IF( Empty( cRd6Fil ) , Replicate( "@" , FWGETTAMFILIAL ) , cRd6Fil )
	cXmCodigo 		+= GetMemVar( "RD6_CODIGO" )
	cCodAux			:= cXmCodigo

	aRd6SxmInfo[ 01 , 01 ] := cRd6Fil
	aRd6SxmInfo[ 01 , 02 ] := cXmCodigo
	aRd6SxmInfo[ 01 , 03 ] := "SIGAAPD/APDA270Sch"
	aRd6SxmInfo[ 01 , 04 ] := GetMemVar( "RD6_DESC" )
	aRd6SxmInfo[ 01 , 05 ] := 1
	aRd6SxmInfo[ 01 , 06 ] := GetMemVar( "RD6_DTINI" )
	aRd6SxmInfo[ 01 , 07 ] := "00:00"
	aRd6SxmInfo[ 01 , 08 ] := GetMemVar( "RD6_DTFIM" )
	aRd6SxmInfo[ 01 , 09 ] := "23:59"
	aRd6SxmInfo[ 01 , 10 ] := "04:00"
	aRd6SxmInfo[ 01 , 11 ] := IF( MsDate() > GetMemVar( "RD6_DTINI" ) , MsDate() , GetMemVar( "RD6_DTINI" ) )
	aRd6SxmInfo[ 01 , 12 ] := SubStr( Time() , 1 , 5 )
	aRd6SxmInfo[ 01 , 13 ] := GetEnvServer()
	aRd6SxmInfo[ 01 , 14 ] := "APDA270Sch('" + cEmpAnt + "' , '" + cRd6Fil + "' , '" + GetMemVar( "RD6_CODIGO" ) + "')"
	aRd6SxmInfo[ 01 , 15 ] := cXMAtivo
	aRd6SxmInfo[ 01 , 16 ] := "F"

	cXmCodigo		:= "APDC" + IF( Empty( cRd6Fil ) , Replicate( "@" , FWGETTAMFILIAL ) , cRd6Fil )
	cXmCodigo 		+= GetMemVar( "RD6_CODIGO" )

	aRd6SxmInfo[ 02 , 01 ] := cRd6Fil
	aRd6SxmInfo[ 02 , 02 ] := cXmCodigo
	aRd6SxmInfo[ 02 , 03 ] := "SIGAAPD/ApdChkEnv"
	aRd6SxmInfo[ 02 , 04 ] := GetMemVar( "RD6_DESC" )
	aRd6SxmInfo[ 02 , 05 ] := 1
	aRd6SxmInfo[ 02 , 06 ] := GetMemVar( "RD6_DTINI" )
	aRd6SxmInfo[ 02 , 07 ] := "00:00"
	aRd6SxmInfo[ 02 , 08 ] := GetMemVar( "RD6_DTFIM" )
	aRd6SxmInfo[ 02 , 09 ] := "23:59"
	aRd6SxmInfo[ 02 , 10 ] := "04:00"
	aRd6SxmInfo[ 02 , 11 ] := IF( MsDate() > GetMemVar( "RD6_DTINI" ) , MsDate() , GetMemVar( "RD6_DTINI" ) )
	aRd6SxmInfo[ 02 , 12 ] := SubStr( Time() , 1 , 5 )
	aRd6SxmInfo[ 02 , 13 ] := GetEnvServer()
	aRd6SxmInfo[ 02 , 14 ] := "ApdChkEnv('" + cEmpAnt + "' , '" + cRd6Fil + "')"
	aRd6SxmInfo[ 02 , 15 ] := cXMAtivo
	aRd6SxmInfo[ 02 , 16 ] := "F"

	nLoops := Len( aRd6SxmInfo )
	For nLoop := 1 To nLoops
		cRd6Fil		:= aRd6SxmInfo[ nLoop , 01 ]
		cXmCodigo	:= aRd6SxmInfo[ nLoop , 02 ]
		cLstCodigo	:= cXmCodigo
		cCodAux		:= cXmCodigo
		IF Eval( bGetNrExclOk )
			lFound := Rd6SeekSxm( cRd6Fil , cXmCodigo )
			IF !( lDelete )
				lAddNew	:= ( ( cLstCodigo == cXmCodigo ) .and. !( lFound ) )
				Rd6PutSxmI( aRd6SxmInfo[ nLoop ] , lAddNew )
			ElseIF ( lFound )
				Rd6DelSxm( cRd6Fil , cXmCodigo )
			EndIF
		EndIF
	Next nLoop
...
/*/
зддддддддддбддддддддддддддддбдддддбдддддддддддддддддддбддддддбдддддддддд©
ЁFun┤┘o    ЁRd6PutSxmI		ЁAutorЁMarinaldo de Jesus Ё Data Ё16/10/2004Ё
цддддддддддеддддддддддддддддадддддадддддддддддддддддддаддддддадддддддддд╢
ЁDescri┤┘o ЁGrava as Informacoes no SXM   								Ё
цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
ЁSintaxe   Ё<Vide Parametros Formais>									Ё
цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
ЁParametrosЁ<Vide Parametros Formais>									Ё
цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
ЁUso       ЁRd6PutSxm em APDA270()                                      Ё
юддддддддддадддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
Static Function Rd6PutSxmI( aRd6SxmInfo , lAddNew )

Local lPutSxmOk

IF ( lPutSxmOk := SXM->( RecLock( "SXM" , lAddNew ) ) )
	SXM->XM_FILIAL	:= aRd6SxmInfo[ 01 ]
	SXM->XM_CODIGO	:= aRd6SxmInfo[ 02 ]
	SXM->XM_NOME	:= aRd6SxmInfo[ 03 ]
	SXM->XM_DESCR	:= aRd6SxmInfo[ 04 ]
	SXM->XM_TIPO	:= aRd6SxmInfo[ 05 ]
	SXM->XM_DTINI	:= aRd6SxmInfo[ 06 ]
	SXM->XM_HRINI	:= aRd6SxmInfo[ 07 ]
	SXM->XM_DTFIM	:= aRd6SxmInfo[ 08 ]
	SXM->XM_HRFIM	:= aRd6SxmInfo[ 09 ]
	SXM->XM_INTERV	:= aRd6SxmInfo[ 10 ]
	SXM->XM_DTPROX	:= aRd6SxmInfo[ 11 ]
	SXM->XM_HRPROX	:= aRd6SxmInfo[ 12 ]
	SXM->XM_AMBIENT	:= aRd6SxmInfo[ 13 ]
	SXM->XM_ACAO	:= aRd6SxmInfo[ 14 ]
	SXM->XM_ATIVO	:= aRd6SxmInfo[ 15 ]
	SXM->XM_TIMEOUT	:= aRd6SxmInfo[ 16 ]
	SXM->( MsUnLock() )
	SXM->( FkCommit() )
EndIF
...
/*/
зддддддддддбддддддддддддддддбдддддбдддддддддддддддддддбддддддбдддддддддд©
ЁFun┤┘o    ЁRd6DelSxm		ЁAutorЁMarinaldo de Jesus Ё Data Ё16/10/2004Ё
цддддддддддеддддддддддддддддадддддадддддддддддддддддддаддддддадддддддддд╢
ЁDescri┤┘o ЁDeleta Informacoes no SXM   								Ё
цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
ЁSintaxe   Ё<Vide Parametros Formais>									Ё
цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
ЁParametrosЁ<Vide Parametros Formais>									Ё
...
ЁUso       ЁRd6PutSxm em APDA270()                                      Ё
юддддддддддадддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
Static Function Rd6DelSxm( cRd6Fil , cXmCodigo )

Local aSxmRecnos	:= {}
Local aSxmScope		:= {}	

IF Rd6SeekSxm( cRd6Fil , cXmCodigo )
	aSxmRecnos := { SXM->( Recno() ) }
Else
	CREATE SCOPE aSxmScope FOR ( ( XM_FILIAL == cRd6Fil ) .and. ( XM_CODIGO == cXmCodigo ) )
	SXM->( ScopeCount( aSxmScope , "SXM" , @aSxmRecnos , .T. ) )
EndIF
DelRecnos( "SXM" , @aSxmRecnos )

Return( NIL )

/*/
зддддддддддбддддддддддддддддбдддддбдддддддддддддддддддбддддддбдддддддддд©
ЁFun┤┘o    ЁRd6SeekSxm		ЁAutorЁMarinaldo de Jesus Ё Data Ё16/10/2004Ё
цддддддддддеддддддддддддддддадддддадддддддддддддддддддаддддддадддддддддд╢
ЁDescri┤┘o ЁProcura Informacoes no SXM   								Ё
цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
ЁSintaxe   Ё<Vide Parametros Formais>									Ё
цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
ЁParametrosЁ<Vide Parametros Formais>									Ё
цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
ЁUso       ЁRd6PutSxm e Rd6DelSxm em APDA270()                          Ё
юддддддддддадддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
Static Function Rd6SeekSxm( cRd6Fil , cXmCodigo )

lFound := ( Upper( AllTrim( SXM->( IndexKey() ) ) ) == "XM_FILIAL+XM_CODIGO" )
IF ( lFound )
	lFound := SXM->( MsSeek( cRd6Fil + cXmCodigo , .F. ) )
EndIF
...
ЁUso       ЁAPDA270()													Ё
юддддддддддадддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
Static Function Rd9Rdh2RdaChg( nFolder , lTransToAll )
...
ЁUso       ЁAPDA270()													Ё
юддддддддддадддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
Static Function GdRdpRd9Chg( lAllToCol , lColToAll , nFolder )
...
MyCursorWait()
	...
	IF !Empty( cIteVis )
		...
		IF ( cRdhNivel == "1" )		//Mesmo Nivel
			cKeyFilter := ApdaDorNivel( cIteVis , cRdeFil , cCodVis , "1" , nNiveis )
		ElseIF ( cRdhNivel == "2" )	//Superior
			cKeyFilter := ApdaDorNivel( cIteVis , cRdeFil , cCodVis , "2" , nNiveis )
		ElseIF ( cRdhNivel == "3" )	//Inferior
			cKeyFilter := ApdaDorNivel( cIteVis , cRdeFil , cCodVis , "3" , nNiveis )
		EndIF
	EndIF
...
ЁUso       ЁScheduler	                                                Ё
юддддддддддадддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
Function APDA270Sch( aParamUser )
...
Begin Sequence
	...
	IF ( Len( aParamUser ) >= 1 )
		cRd6Emp := aParamUser[1]
	EndIF
	IF ( Len( aParamUser ) >= 2 )
		cRd6Filial := aParamUser[2]
	EndIF
	IF ( Len( aParamUser ) >= 3 )
		cRd6CodAva := aParamUser[3]
	EndIF
	IF ( Len( aParamUser ) >= 4 )
		uDataIni := aParamUser[4]
	EndIF
	IF ( Len( aParamUser ) >= 5 )
		uDataFim := aParamUser[5]
	EndIF

End Sequence
	
Return( APDA270Job( cRd6Emp , cRd6Filial , cRd6CodAva , uDataIni , uDataFim ) )

/*/
зддддддддддбддддддддддддддддбдддддбдддддддддддддддддддбддддддбдддддддддд©
ЁFun┤┘o    ЁAPDA270Job		ЁAutorЁMarinaldo de Jesus Ё Data Ё29/07/2004Ё
цддддддддддеддддддддддддддддадддддадддддддддддддддддддаддддддадддддддддд╢
ЁDescri┤┘o ЁChamada do Apada080 atraves de JOB para a Geracao das AvaliaЁ
Ё  		   Ёcoes														Ё
цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
ЁSintaxe   Ё<Vide Parametros Formais>									Ё
цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
...
/*/
зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
Ё Verifica se Passou Data Inicial e Data Final                    		Ё
юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
lChkIniFim := ( ( ValType( "dDataIni" ) == "D" ) .and. ( ValType( "dDataFim" ) == "D" ) )

/*/
зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
Ё Redefine nModulo de forma a Garantir que o Modulo seja o SIGAAPD		Ё
юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
aModuloReSet := SetModulo( "SIGAAPD" , "APD" )

/*/
зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
Ё Carrega Informacoes da Empresa/Filial                            		Ё
юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
aEmpFil := ArrEmpFilGet()

/*/
зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
Ё Verifica Em que Empresa Filial Se iniciara e Findara o Processamento  Ё
юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
IF ( ( lEmp ) .and. ( lRd6Filial ) )
	nInitLoop := aScan( aEmpFil , { |x| ( x[1] == cRd6Emp ) .and. ( x[2] == cRd6Filial ) } )
	IF ( nInitLoop > 0 )
		nLoops := aScan( aEmpFil , { |x| ( x[1] == cRd6Emp .and. x[2] <> cRd6Filial ) } , nInitLoop + 1 )
		IF ( nLoops > 0 )
			--nLoops
		Else
			nLoops := aScan( aEmpFil , { |x| ( x[1] <> cRd6Emp ) } , nInitLoop + 1 )
			IF ( nLoops > 0 )
				--nLoops
			EndIF
		EndIF
	...
	EndIF
ElseIF ( lEmp )
	nInitLoop := aScan( aEmpFil , { |x| ( x[1] == cRd6Emp ) } )
	IF ( nInitLoop > 0 )
		nLoops := aScan( aEmpFil , { |x| ( x[1] <> cRd6Emp ) } , nInitLoop + 1 )
		IF ( nLoops > 0 )
			--nLoops
...
/*/
зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
Ё Processa Para Cada Empresa/Filial                                		Ё
юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
...
For nLoop := nInitLoop To nLoops
	...
	/*/
	зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
	Ё Obtem a Empresa a Ser processada            							Ё
	юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
	...
	/*/
	зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
	Ё Verifico a Existencia dos SXs para a Empresa							Ё
	юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
	IF !RpcChkSxs( cEmp , @aMsgErr )
		Loop
	EndIF
	
	/*/
	зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
	Ё Verifico se Devo Processar a Empresa       							Ё
	юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
	IF ( ( lEmp ) .and. ( cEmp <> cRd6Emp ) )
		Loop
	EndIF

	/*/
	зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
	Ё Obtem a Filial e o Recno do SM0            							Ё
	юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
	cRd6Filial	:= aEmpFil[ nLoop , 02 ]
	nSM0Rec		:= aEmpFil[ nLoop , 03 ]

	/*/
	зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
	Ё Garanto que nao vai Consumir Licencas									Ё
	юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
	RpcSetType( 3 )
	
	PREPARE ENVIRONMENT EMPRESA ( cEmp ) FILIAL ( cRd6Filial )
		...
		Begin Sequence
			...
			/*/
			зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
			Ё Garanto o Posicionamento no SIGAMAT									Ё
			юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
			SM0->( MsGoto( nSM0Rec ) )
			cEmpAnt := cEmp
			cFilAnt := cRd6Filial
			...
			/*/
			зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
			Ё Abre as Tabelas do Ponto												Ё
			юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
			PonRelationFile()
	
			/*/
			зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
			Ё Abre as Tabelas do GPE  												Ё
			юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
            GpeRelationFile()
	
			/*/
			зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
			Ё Abre as Tabelas do APD  												Ё
			юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
			...
			/*/
			зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
			Ё Monta a chave para pesquisa											Ё
			юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
			IF ( lRd6CodAva )
			...
			/*/
			зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
			Ё Executa a Geracao       												Ё
			юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
			While RD6->(;
							!Eof();
							.and.;
							IF( lChkFilAva , SubStr( &( cRd6IndexKey ) , 1 , nRd6KeyLen ) == cRd6KeySeek , .T. );
					   )
				...
				/*/
				зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
				Ё Apenas as Avaliacoes Ativas                                      		Ё
				юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
				IF ( RD6->RD6_STATUS == "1" )
					
					/*/
					зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
					Ё Se nao Passou a Data Considera o Periodo do RD6                  		Ё
					юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
					IF !( lChkIniFim )
		...
		/*/
		зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		Ё Inicializo o Processo de Envio das Avaliacoes                    		Ё
		юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
		IF ( lStartJob )
			StartJob( "ApdSndAv" , GetEnvServer() , .F. , { cEmpAnt , cFilAnt } )
		EndIF	
...
/*/
зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
Ё Mostra no Console as Inconsistencias                             		Ё
юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
ConOut( CRLF )
nLoops := Len( aMsgErr )
For nLoop := 1 To nLoops
...
/*/
зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
Ё Restaura Informacoes de Entrada                                  		Ё
юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
...
//msgalert("Parou.."+strzero(nRdeLoop,3))
						aRdeCodRd0	:= {}
						/*/
						здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
						...
						Else
							ApdaDorNivel(;
												aRdeCols[ nRdeLoop , nRdeIteVis ],;	//01 -> Codigo do Item Inicial
						...
						nLoopRdeRd0 := 0
						nQtLoop := 0
//varinfo("Passo 0 - processando o:",cRd0Codigo)
				
//varinfo("aTotPar",aTotPar)
//varinfo("nLoopsRdeRd0",nLoopsRdeRd0)						
						While .T.
							nLoopRdeRd0 := Randomize( 1, nLoopsRdeRd0+1 )
//varinfo("nLoopRdeRd0....Numero aleatorio:",nLoopRdeRd0)						
							If nLoopRdeRd0 > nLoopsRdeRd0 //DocumentaГЦo diz que funГЦo retorna n?ero no intervalo do parametro inicial at?o parametro final menos 1...por? esta devolvendo o valor do parametro final..inclus?deste IF para n? ocorrer problemas.
//varinfo("Loop 1",'')						
								Loop
							EndIf
							If ( ( nMaxCodDor + 1 ) > nRdhMaxPar )
//varinfo("Exit 1 -> nMaxCodDor:",nMaxCodDor)						
								Exit
							EndIf
					
							//Ponto de entrada para permitir escolha do avaliador
							If ExistBlock( "APD270DOR"	)
								uRet := ExecBlock("APD270DOR", .F. , .F. ,{cRd0Codigo,GetMemVar("RD6_CODPER"),nLoopRdeRd0,aRdeCodRd0,aTotPar,aRdhHeader,aRdhCols,nRdhLoop,aPosAux}) 
								IF ( ValType( uRet  ) == "A" )
								   nLoopRdeRd0   	:= uRet[1]
								   aRdeCodRd0   	:= aClone(uRet[2])
								ElseIF ( ValType( uRet  ) == "N" )
									If uRet == 0 //Despreza o avaliador para que a rotina escolha outro
										If nQtLoop > (nLoopsRdeRd0*3) //Condicao para garantir que nao entre em loop infinito
//varinfo("Exit 2",nQtLoop)						
											exit
										Else 
											nQtLoop++
//varinfo("Loop 2",nQtLoop)						
											loop
										EndIf
									ElseIf uRet == 1 //Despreza o avaliador  e interrompe a escolha
//varinfo("Desprezou com uRet==1",nQtLoop)
										exit
									EndIf
								EndIF
							EndIf							
							...
							While .T.
								If aScan( aPosAux , { |x| x == nLoopRdeRd0 } ) > 0 .or. ( aRdeCodRd0[ nLoopRdeRd0 , 1 ] == cRd0Codigo )
									If lSomaAux
										nLoopRdeRd0++
//varinfo("lSomaAux = .t.",nLoopRdeRd0)						
									Else
										nLoopRdeRd0--
//varinfo("lSomaAux = .f.",nLoopRdeRd0)						
									EndIf
									If nLoopRdeRd0 > nLoopsRdeRd0
										nLoopRdeRd0--
										lSomaAux := .F. //Se foi at?a ?tima posiГЦo, come? a decrementar para encontrar as menores que na?foram usadas
//varinfo("nLoopRdeRd0 > nLoopsRdeRd0",nLoopRdeRd0)						
									EndIf
									If nLoopRdeRd0 == 0
//varinfo("Exit 3",nLoopRdeRd0)						
									 	Exit
									EndIf
//varinfo("Loop 3",'')						
									Loop
...
//varinfo("aPosAux",aPosAux)						
									Exit
								EndIf
							EndDo
							If nLoopRdeRd0 > nLoopsRdeRd0 .or. nLoopRdeRd0 == 0 .or. Len(aPosAux) > nLoopsRdeRd0 
If nLoopRdeRd0 > nLoopsRdeRd0
//varinfo("nLoopRdeRd0 > nLoopsRdeRd0",'')
EndIf
If nLoopRdeRd0 == 0
//varinfo("nLoopRdeRd0 == 0",'')
EndIf
If Len(aPosAux) > nLoopsRdeRd0 
//varinfo("Len(aPosAux) > nLoopsRdeRd0",'')
EndIf
//varinfo("Exit 4",'')						
								Exit
							EndIf
...
//varinfo("Loop 4",'')						
								Loop
							EndIf
							//Se j?foi avaliador mais do que a quantidade m?ia, pula (apenas se as opГУes j?n? estiverem acabando)
...
Local aGrupos	:= UsrRetGrp( SubStr( cUsuario , 7 , 15 ) )
Local cCodGrp	:= ""
...
Begin Sequence
	...
	IF ( GetMemVar( "RD6_MONTAG" ) == "1" )
		IF !( lRd6CriterVld := Vazio() )
			...
			Help( "" , 1 , "INCONSISTENCIA" , NIL , OemToAnsi( STR0093 ) , 1 , 0 )
			Break
		Else
			Break
		EndIF
	...
	Else
		...
		lRd6CriterVld := .F.
			...
			Help( "" , 1 , "INCONSISTENCIA" , NIL , OemToAnsi( "Este campo deverА ser preenchido quando a Montagem for AutomАtica ou Semi-AutomАtica" ) , 1 , 0 )
			Break
		Else
			Break
		EndIF
...
IF !( lValid )
	...
	Else
		...
		Do Case
			...
			Case ( cOpcDesc == "2" ) ; ( cOpcBox := STR0133 )	//"Avaliacao"
