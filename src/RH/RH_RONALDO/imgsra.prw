...
Begin Sequence
	...
	aFilesJPG1	:= Directory( cPathFile )
	
	If Len(aFilesJPG1) > 0
		For n:=1 to Len(aFilesJPG1)
			aadd(aFilesJPG, aFilesJPG1[n,1])
		Next
	EndIf	
	
	nFilesJPG	:= Len(aFilesJPG)
	...
	aFilesJPEG1	:= Directory( cPathFile )
	
	If Len(aFilesJPEG1) > 0
		For n:=1 to Len(aFilesJPEG1)
			aadd(aFilesJPEG, aFilesJPEG1[n,1])
		Next
	EndIf	

	nFilesJPEG	:= Len(aFilesJPEG)
	...
	aFilesBMP1	:= Directory( cPathFile )
	
	If Len(aFilesBMP1) > 0
		For n:=1 to Len(aFilesBMP1)
			aadd(aFilesBMP, aFilesBMP1[n,1])
		Next
	EndIf	

	nFilesBMP	:= Len(aFilesBMP)
...
Begin Sequence
	...
	юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды/*/
	lPutOk	:= ( Alltrim(cSraBitMap) == Alltrim(oRepository:InsertBmp( cPathFile , NIL , @lPut )) )
