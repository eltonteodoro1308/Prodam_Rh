...
*/
USER Function PRGPER01(lTerminal,cFilTerminal,cMatTerminal,cProcTerminal, nRecTipo, cPerTerminal, cSemanaTerminal)                 
...
nTipRel    := IF( !( lTerminal ), IIF(mv_par05==1,4,5) , 3	)	//Tipo de Recibo (Pre-Impressão/Zebrado/EMail/PDF)
...
oPrint:Say(2180,1090,SRA->RA_CTDEPSAL,oArial08N)	
oPrint:Say(2260,0040,STR0165,oArial10N)//Declaro que Recebi o valor acima referido em ___/___/___
...
If cPaisLoc <> "ARG"
	...
	@ LI,aPos[2] PSAY STR0033+SRA->RA_BCDEPSA+"-"+Substr(DescBco(SRA->RA_BCDEPSA,SRA->RA_FILIAL), 1, nTam)	//"| CREDITO:"
	nTam := 43 - (Len(STR0034) + Len(TRANS((TOTVENC-TOTDESC),cPict1)))
	@ LI,aPos[4] PSAY STR0034+SPACE(nTam-1)+TRANS((TOTVENC-TOTDESC),cPict1)			//"| LIQUIDO A RECEBER     "
	@ LI,131 PSAY "|"
...
IF SRA->RA_BCDEPSAL # SPACE(8)
	Desc_Bco := If( lLpt1, fTAcento(DescBco(Sra->Ra_BcDepSal,Sra->Ra_Filial)), DescBco(Sra->Ra_BcDepSal,Sra->Ra_Filial) )
...
Local cFileLogo	:= "\web\PP\imagens-rh\topo_logo-esq.jpg" //"LGRL"+FwCodEmp("SM0")+".bmp"
Local cCodFunc	:= ""		//-- Codigo da Funcao do funcionario
Local cDescFunc	:= ""		//-- Descricao da Funcao do Funcionario
Local cDescCC		:= ""		//-- Descricao do Centro de Custo
Local nLi			:= 0
...
nLi += 10
oPrinter:Box(  nLi-8, 004, nLi+20, 585, "-4" )			// Caixa da linha 
oPrinter:SayBitmap(nLi-7,008,cFileLogo,50,25)

nLi += 10
oPrinter:SayAlign(nLi-15,004,STR0017,oFont4,585,005, /**/, 2, 1 ) //"RECIBO DE PAGAMENTO  "
nLi += 10
oPrinter:Box(  nLi+5, 004, nLi+55, 585, "-4" )			// Caixa da linha
nLi += 20
...
Local nLinBas		:= nPosFCab
	...
	For nCont := 1 To nTermina
		...
		//Imprime Base
		IF nCont <= LEN(aBases) 
			If cPortal == "N" .And. cBaseAux = "S" .And. nBaseIr # 0
			...
			//Tratamento especifico Prodam para imprimir as bases igual ao demosntrativo do portal RH
			ElseIf cPortal == "S" .And. cBaseAux = "S" .And. nBaseIr # 0; 
					.And. LEFT(aBases[nCont,1],3)$aCodFol[10,1]+'*'+aCodFol[13,1]+'*'+;
							aCodFol[221,1]+'*'+aCodFol[19,1]+'*'+aCodFol[108,1]+'*'+aCodFol[17,1]+'*'+; 
							aCodFol[337,1]+'*'+aCodFol[398,1]+'*'+aCodFol[109,1]+'*'+;
							aCodFol[18,1]+'*'+aCodFol[339,1]+'*'+aCodFol[400,1]+'*'+aCodFol[15,1]
				nLinBas+= 10
				oPrinter:SayAlign(nLinBas,aPos[6],aBases[nCont,1],oFont1,390,005,,ALIGN_H_LEFT)
				oPrinter:SayAlign(nLinBas,aPos[6],TRANSFORM(aBases[nCont,2],'999.99'),oFont1,130,005,,ALIGN_H_RIGHT)
				oPrinter:SayAlign(nLinBas,aPos[6],TRANSFORM(aBases[nCont,3] , cPict3 ),oFont1,192,005,,ALIGN_H_RIGHT)
			EndIf
		ENDIF
	...
	oPrinter:SayAlign(nLi-10,aPos[4],Space(2)+Substr(STR0033,3)+ Space(2) + AllTrim(SRA->RA_BCDEPSAL)+" - "+substr(DescBco(SRA->RA_BCDEPSAL,SRA->RA_FILIAL),1,25),oFont2,196,005, , 0, 1 )  	//"| CREDITO:"
	...
	oPrinter:SayAlign(nLi-10,aPos[4],Space(2)+UPPER(STR0040)+ Space(2)+SRA->RA_CTDEPSAL,oFont2,196,005, , 0, 1 )   //"| CONTA:"
	...
	For nI := 1 To Len(aEmail)
		...
		If aEmail[nI,1,3]
			If ! Empty(AllTrim(aEmail[nI,1,2]))
				cSubject	+= " - " + MesExtenso(MONTH(SToD(aEmail[nI,4])))+"/"+STR(YEAR(StoD(aEmail[nI,4])),4)
